...
bool Sudoku::solve() {
    ...
    // If there is no empty cell, we did it!
    if (!find_empty_location(row, col)) { return true; }

    // Check all digits 1-9
    for (int num = 1; num <= 9; num++) {
        // If there are no conflicts, place & recurse
        if (is_valid(row, col, num)) {
            (*this->board)[row][col] = num;

            // If this returns true, there were no conflicts
            // in any subsequent recursive calls
            if (solve()) { return true; }

            // There was a conflict, try placing the next number.
